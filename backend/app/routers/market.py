...
def overview(region: str = Query("all")) -> MarketOverview:
    ...
    return MarketOverview(
        ...
        window_days=90, updated_at="seed",
...
def skill_gaps(region: str = Query("all")) -> SkillGapResponse:
    ...
    return SkillGapResponse(region=region, skills=top)


@router.get("/careers/{career_id}")
def career_detail(career_id: str, region: str = Query("all")) -> dict:
    c = get_career(career_id)
    if not c:
        raise HTTPException(404, detail="career not found")
    m = dict(c["seed_market"])
    m["source_note"] = SEED_NOTE
    return {"career_id": c["career_id"], "title": c["title"], "description": c["description"],
            "market": m, "routes": c["routes"]}
