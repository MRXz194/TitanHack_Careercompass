...
def get_overview(region: str) -> MarketOverview:
    ...
    return MarketOverview(
        ...
        rising_careers=[
            # trend_pct=None: single-snapshot crawl (see module docstring), so there is no real
            # trend signal for ANY career, regardless of demand sample size. Do not substitute a
            # sentinel number here (0.0 previously leaked through the FE as a literal "▲0%" for
            # well-sampled careers, since the FE only gates on low_confidence, not on trend_pct
            # itself) — None makes "no trend data" and "well-sampled demand" independently true.
            # low_confidence is the real demand-sample-size signal (< 10 postings), unrelated to
            # trend availability.
            RisingCareer(career_id=r["career_id"], title=_title_for(r["career_id"]),
                        trend_pct=None, demand_count=r["demand_count"],
