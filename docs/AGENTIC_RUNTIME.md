...
### Runtime đã chọn: LangChain + LangGraph tối giản

MVP dùng LangChain Core/`langchain-openai` cho model adapters, Pydantic structured output và typed tool contracts; dùng LangGraph `StateGraph` làm lớp điều phối node/conditional edge cho `/api/chat`. Xem quyết định, version pins và spike gate tại `ADR_AGENT_ORCHESTRATION.md`. LangGraph không sở hữu domain rules hay session state; graph nhận state đã sanitize, gọi policy/tool hiện có và trả update để service persist. Mọi model/embedding instance chỉ được tạo trong `services/llm.py`.
...
Mọi tool là hàm backend typed, idempotent nếu có thể, khai báo bằng LangChain `@tool(args_schema=...)` và đăng ký qua `AgentToolRegistry`. Decorator chỉ tạo schema/callable; CareerCompass policy vẫn authorize trước/sau execution. Không có browser, shell, code execution, write DB tổng quát hay external side effect trong MVP.

```python
from langchain_core.tools import tool
from pydantic import BaseModel, Field

class InspectProfileGapsInput(BaseModel):
    session_id_hash: str = Field(description="Opaque session reference; never raw PII")
    stage: AgentStage

@tool(args_schema=InspectProfileGapsInput)
def inspect_profile_gaps(session_id_hash: str, stage: AgentStage) -> dict:
    """Return missing evidence slots for the current allowed profiling stage."""
    return profiler.inspect_gaps(session_id_hash=session_id_hash, stage=stage)
```

Đây là implementation example; schema canonical đặt ở backend models/tool module theo ADR.
...
|---|---|---|
| M1 | feature flag, dependency/CI, replay, release gate | pinned LangChain/LangGraph install, `AGENT_MODE=langgraph|deterministic`, recorded safe trace fixtures, go/no-go |
