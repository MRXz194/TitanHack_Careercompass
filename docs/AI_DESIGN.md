...
## 3. Market Stats & Skill Gap Index (M3)

Mỗi (career × region) và (skill × region), cửa sổ 90 ngày:
- `demand_count` = số postings.
- `salary_p25/p50/p75` = percentile trên các posting CÓ lương (median của range từng posting); < 5 mẫu → trả null, FE hiện "chưa đủ dữ liệu" — **không bịa**.
- `trend_pct` = (count 45 ngày cuối − count 45 ngày đầu) / max(count 45 ngày đầu, 5) × 100. Mẫu < 10 → gắn `low_confidence: true`.

**Skill Gap Score** (0..1, mỗi region):
```
gap = 0.5·norm(demand_count) + 0.3·norm(max(trend_pct,0)) + 0.2·scarcity_hint
```
`scarcity_hint`: tỉ lệ posting yêu cầu skill này mà vẫn đăng lại/kéo dài >30 ngày (proxy "khó tuyển"); nếu không tính được từ dữ liệu → dùng 0.5·norm(demand)+0.5·norm(trend) và ghi chú công thức trên UI. **Nguyên tắc: công thức đơn giản mà giải thích được > công thức phức tạp mà ú ớ khi judge hỏi.**
...
| Việc | Model | Ghi chú |
|---|---|---|
| Chat profiling, evidence | DeepSeek chat (OpenAI-compatible, `CHAT_API_BASE` + `CHAT_MODEL`) | Test chất lượng TV + JSON ngay Phase 1 (PR-02) |
