...
Status: **acquisition complete, runtime aggregate not yet approved**. Snapshot này là input cho N2-01/N3-01; không được copy thẳng số liệu salary/region/trend lên UI trước normalization, mapping và quality gate.
...
5. **Normalizer safety/performance:** script hiện không có safe `--help`/`--dry-run`, output chưa atomic và fuzzy dedupe quét O(n²). N2-01 phải thêm CLI/preflight/atomic output và blocking keys trước full rebuild.
...
1. Không chạy `normalize.py --help` trên version hiện tại vì script bắt đầu pipeline; sửa CLI/preflight trước.
2. Pin ba raw file bằng cả canonical hash và file hash ở trên.
3. Implement source-aware salary/region/date validity, content/event dedupe và atomic output.
4. Xuất manifest theo `DATA_RESEARCH_ARCHITECTURE.md`, coverage/error report và sanitized trace; không commit enriched raw descriptions.
5. Handoff artifact/hash/limitations cho M3; M3 chỉ build `market.db` khi quality gates pass.
