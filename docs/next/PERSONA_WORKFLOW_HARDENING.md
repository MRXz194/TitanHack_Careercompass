...
| PH-14 | Sev-1 verification | Backend host/Python không có trên máy audit | Không thể gọi local backend/pytest | Chờ CI/máy backend |
...
- Backend new tests: NOT RUN local (máy không có Python); phải qua CI/máy backend.
...
Release status: **candidate hardening, chưa final-ready** cho tới khi bốn dòng manual/CI còn mở
được cập nhật bằng evidence thật.
